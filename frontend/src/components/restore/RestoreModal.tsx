...
export default function RestoreModal({ open, onClose, backup, connections, onSuccess }: Props) {
  const [targetConnId, setTargetConnId] = useState("");
  const [tables, setTables]             = useState("");       // comma separated
  ...
  const handleRestore = async () => {
    if (!backup) return;
    if (!targetConnId) { setError("select target connection"); return; }
    setError("");
    setRunning(true);
    ...
    <Modal open={open} onClose={handleClose} title="restore_backup">
      {!backup ? null : running ? (
        <div className="space-y-5">
          <p className="text-xs" style={{ color: "#8a9690" }}>restoring from {backup.filename}</p>
          <ProgressBar percent={cur?.pct ?? 0} stage={cur?.label} />
          <div className="space-y-1">
            {STAGES.map((s, i) => (
              <div key={i} className="flex items-center gap-2 text-xs">
                <span style={{ color: i < stageIdx ? "#4ade80" : i === stageIdx ? "#b8f53a" : "#4a5450" }}>
                  {i < stageIdx ? "✓" : i === stageIdx ? "›" : "○"}
                </span>
                <span style={{ color: i < stageIdx ? "#4ade80" : i === stageIdx ? "#e8edea" : "#4a5450" }}>
                  {s.label}
                </span>
              </div>
            ))}
        ...
        <div className="space-y-4">
          {/* Backup info */}
          <div className="px-3 py-2.5 rounded text-xs space-y-1" style={{ background: "#1a1d1a", border: "1px solid #252825" }}>
          ...
          <div className="space-y-1.5">
            <label className="text-xs" style={{ color: "#4a5450" }}>target_connection</label>
            <select
          ...
          <div className="space-y-1.5">
            <label className="text-xs" style={{ color: "#4a5450" }}>
              selective_tables{" "}
              <span style={{ color: "#3d4040" }}>(optional — comma separated, empty = full restore)</span>
            </label>
            <input
              className="terminal-input"
              type="text"
          ...
          {/* Warning */}
          <div className="px-3 py-2.5 rounded text-xs" style={{ background: "rgba(255,215,0,0.05)", border: "1px solid rgba(255,215,0,0.15)", color: "#ffd700" }}>
            ⚠ this will overwrite existing data in the target database
          </div>

          <button onClick={handleRestore} className="btn-acid w-full">
            $ restore_now →
          </button>
        </div>
      )}

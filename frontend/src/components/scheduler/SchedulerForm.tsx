...
import type { DbConnection, CreateScheduleDto, ScheduleFrequency, BackupType } from "@/types";
...
export default function SchedulerForm({ open, onClose, connections, onSuccess }: Props) {
          ...
          <div className="grid grid-cols-2 gap-2">
            {FREQ_OPTIONS.map((f) => (
              <button
                ...
                style={{
                  borderColor: form.frequency === f.value ? "#b8f53a" : "#252825",
                  background: form.frequency === f.value ? "rgba(184,245,58,0.06)" : "transparent",
                }}
              >
                <p className="text-xs font-semibold" style={{ color: form.frequency === f.value ? "#b8f53a" : "#e8edea" }}>
                  {f.label}
                </p>
                <p className="text-xs mt-0.5" style={{ color: "#4a5450" }}>{f.desc}</p>
              </button>
            ))}
          </div>
        </div>

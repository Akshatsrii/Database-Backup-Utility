...
export default function LiveLogTerminal({ initialLogs = [], height = 480 }: Props) {
  ...
  const filtered = logs.filter((l) => {
    ...
    const srchOk = !search || l.message.toLowerCase().includes(search.toLowerCase());
    return lvlOk && srchOk;
  });

  const levelDot: Record<string, string> = {
    info: "#38bdf8", warn: "#ffd700", error: "#ff4444", success: "#4ade80", debug: "#4a5450",
  };
  ...
  return (
    <div className="terminal-card flex flex-col" style={{ height }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="flex items-center gap-1.5 text-xs">
            <span
              ...
              style={{ background: connected ? "#4ade80" : "#ff4444", boxShadow: connected ? "0 0 4px #4ade80" : "none" }}
            />
            <span style={{ color: "#4a5450" }}>{connected ? "live" : "disconnected"}</span>
          </span>
          ...
          <input
            ...
            style={{ background: "#1a1d1a", border: "1px solid #252825", color: "#e8edea", width: 140, outline: "none" }}
          />
          ...
          <select
            ...
            className="text-xs px-2 py-1 rounded"
            style={{ background: "#1a1d1a", border: "1px solid #252825", color: "#e8edea", appearance: "none" }}
          >
      ...
      {/* Log lines */}
      <div className="flex-1 overflow-y-auto p-4 font-mono text-xs space-y-0.5" style={{ background: "#0a0c0a" }}>
        {filtered.length === 0 && (
          <p style={{ color: "#3d4040" }}>waiting for logs... {connected ? "●" : "○"}</p>
        )}
        {filtered.map((log) => (
          <div key={log.id} className="flex gap-3 leading-5 hover:bg-bg-tertiary rounded px-1">
      ...
      >
        ...
        <span className="text-xs cursor-blink" style={{ color: "#3d4040" }}>
          {paused ? "PAUSED" : "LIVE"} ▌
        </span>
      </div>
    </div>

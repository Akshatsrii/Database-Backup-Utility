...
import type { ApiResponse, CreateConnectionDto, DbConnection } from "../types";
...
export const connectionController = {
  ...
  create(req: Request, res: Response) {
    ...
    res.status(201).json({ success: true, data: safe, message: "Connection created" });
  },

  // PUT /api/connections/:id
  update(req: Request, res: Response) {
    const { id } = req.params;
    const dto = req.body as Partial<CreateConnectionDto>;
    ...
    connectionStore[idx] = { ...connectionStore[idx], ...dto };
    logger.info(`Connection updated: ${id}`);
    
    const { password, ...safe } = connectionStore[idx];
    res.json({ success: true, data: safe, message: "Connection updated" });
  },
